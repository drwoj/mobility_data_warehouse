...
def main_etl():
    ...
    t.convert_rmb_per_ton_to_euros_per_liter(df_fuel_prices_beijing, 0.13, 730)
    t.convert_euros_per_hectolitre_to_euros_per_liter(df_fuel_prices_hannover)
    ...
    df_districts = e.extract_districts()

    t.country_to_city(df_trajectories)

    df_list = [df_weather, df_economy_indicators, df_fuel_prices]
    for df in df_list:
        df['id'] = df.index + 1
    ...
    df_districts['area_polygon'] = df_districts['area'].apply(wkt.loads)
    gdf_districts = gpd.GeoDataFrame(df_districts,
                                        geometry=df_districts['area_polygon'],
                                        crs='EPSG:4326') \
        .drop(columns={'area', 'area_polygon'}) \
    ...
    with MobilityDWConnector() as connector:
        ...
        for index, row in df_trajectories.iterrows():
            ...
            economy_indicator_id_value = row['economy_indicator_id'] if not pd.isna(
                row['economy_indicator_id']) else 'NULL'
